...
- (instancetype _Nonnull)initWithName:(NSString *)name
    ...
    QGCDTimer *timer = [self initWithInterval:interval
                                     delaySecs:delaySecs
                                         queue:queue
                                       repeats:repeats
                                        action:action];
...
@implementation QGCDTimerManager

#pragma mark - 初始化
//第1步: 存储唯一实例
static QGCDTimerManager *_manager = nil;
...
+ (void)scheduledTimerWithName:( NSString * _Nonnull)name
    ...
    QGCDTimer *GCDTimer = [self timer:string];
    if (GCDTimer) {
        return;
    }
    GCDTimer = [[QGCDTimer alloc] initWithName:string
                                       interval:interval
                                      delaySecs:delaySecs
                                          queue:queue
                                        repeats:repeats
...
//MARK:JmoVxia---获取定时器
+ (QGCDTimer *_Nullable)timer:(NSString *_Nonnull)name {
    dispatch_semaphore_wait([QGCDTimerManager sharedManager].semaphore, DISPATCH_TIME_FOREVER);
    __strong NSString *string = name;
    QGCDTimer *GCDTimer = [[QGCDTimerManager sharedManager].timerObjectCache objectForKey:string];
    dispatch_semaphore_signal([QGCDTimerManager sharedManager].semaphore);
    return GCDTimer;
}
//MARK:JmoVxia---储存定时器
+ (void)setTimer:(QGCDTimer *_Nonnull)timer name:(NSString *_Nonnull)name {
    dispatch_semaphore_wait([QGCDTimerManager sharedManager].semaphore, DISPATCH_TIME_FOREVER);
    [[QGCDTimerManager sharedManager].timerObjectCache setObject:timer forKey:name];
    dispatch_semaphore_signal([QGCDTimerManager sharedManager].semaphore);
}
//MARK:JmoVxia---移除定时器
+ (void)removeTimer:(NSString *_Nonnull)name {
    dispatch_semaphore_wait([QGCDTimerManager sharedManager].semaphore, DISPATCH_TIME_FOREVER);
    [[QGCDTimerManager sharedManager].timerObjectCache removeObjectForKey:name];
    dispatch_semaphore_signal([QGCDTimerManager sharedManager].semaphore);
}
@end
