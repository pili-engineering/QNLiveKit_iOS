...
typedef NS_ENUM(NSUInteger, STGLShaderType)
{
    STGLShaderVertex,
    STGLShaderTypeFragment,
    STGLShaderTypeProgram,
};

@interface STGLShader ()
@property (nonatomic, strong) NSMutableArray *attributes;
@property (nonatomic, copy) NSString *vertexShader;
@property (nonatomic, copy) NSString *fragmentShader;
@end

@implementation STGLShader
...
- (void)compileVertexShader:(NSString *)vertexShader fragmentShader:(NSString *)fragmentShader
{
    ...
    [self checkCompileErrors:verShader type:STGLShaderVertex];
    ...
    [self checkCompileErrors:fragShader type:STGLShaderTypeFragment];
    ...
    [self checkCompileErrors:_program type:STGLShaderTypeProgram];
    
    glDeleteProgram(verShader);
    glDeleteShader(fragShader);
}
- (void)checkCompileErrors:(GLuint)shader type:(STGLShaderType)type
{
    GLint success;
    GLchar infoLog[1024];
    if (STGLShaderTypeProgram != type) {
