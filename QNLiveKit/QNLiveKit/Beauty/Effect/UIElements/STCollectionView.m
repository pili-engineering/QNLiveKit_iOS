...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.modelType == STEffectsTypeBeautyWholeMakeup) {
        cell.shapeLayer.hidden = true;
        cell.valueLabel.text = @"";
    }else {
        cell.shapeLayer.hidden = false;
        cell.valueLabel.text = [NSString stringWithFormat:@"%d", model.beautyValue];
    }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.modelType == STEffectsTypeBeautyWholeMakeup) {
        if (model.selected) {
            for (int i = 0; i < self.models.count; i++) {
                STNewBeautyCollectionViewModel *tempM = self.models[i];
                if (tempM.modelType == STEffectsTypeBeautyWholeMakeup && model.beautyType != tempM.beautyType) {
                    tempM.selected = false;
                }
            }
        }
    }
    
    if (model.modelType == STEffectsTypeBeautyBase ||
        model.modelType == STEffectsTypeBeautyShape ||
        model.modelType == STEffectsTypeBeautyMicroSurgery ||
        model.modelType == STEffectsTypeBeautyAdjust) {
        for (int i = 0; i < self.models.count; i++) {
            STNewBeautyCollectionViewModel *tempM = self.models[i];
            if (tempM.modelType == model.modelType && model.beautyType != tempM.beautyType) {
                tempM.selected = false;
            }
        }
    }
...
STNewBeautyCollectionViewModel* getModel(UIImage *normalImage,
                                         UIImage *highlightImage,
                                         NSString *title,
                                         int beautyValue,
                                         BOOL selected,
                                         int modelIndex,
                                         STEffectsType modelType,
                                         STBeautyType beautyType) {
