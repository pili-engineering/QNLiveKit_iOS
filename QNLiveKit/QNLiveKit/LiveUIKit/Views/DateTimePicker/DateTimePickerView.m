...
@interface DateTimePickerView()<UIGestureRecognizerDelegate>
{
...
- (UIView *)pageAtIndex:(NSInteger)index andScrollView:(MXSCycleScrollView *)scrollView
{
    ...
    {
        if (index + 1 < 10) {
            l.text = [NSString stringWithFormat:@"0%ld月",(long)(1 + index)];
        } else {
    ...
    {
        if (index + 1 < 10) {
            l.text = [NSString stringWithFormat:@"0%ld日",(long)(1 + index)];
        } else {
