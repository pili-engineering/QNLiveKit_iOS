...
#import "NSObject+QNProperty.h"
#import "NSString+QNExtension.h"
#import "QNProperty.h"
#import "QNPropertyType.h"
#import "QNExtensionConst.h"
#import "QNFoundation.h"
#import "NSString+QNExtension.h"
#import "NSObject+QNClass.h"

@implementation NSDecimalNumber(QNKeyValue)
...
@implementation NSObject (QNKeyValue)
...
- (instancetype)mj_setKeyValues:(id)keyValues context:(NSManagedObjectContext *)context
{
    ...
    [clazz mj_enumerateProperties:^(QNProperty *property, BOOL *stop) {
        @try {
            // 0.检测是否被忽略
            ...
            for (NSArray *propertyKeys in propertyKeyses) {
                value = keyValues;
                for (QNPropertyKey *propertyKey in propertyKeys) {
            ...
            // 2.复杂处理
            QNPropertyType *type = property.type;
...
+ (NSMutableArray *)mj_objectArrayWithKeyValuesArray:(id)keyValuesArray context:(NSManagedObjectContext *)context
{
    ...
    if ([QNFoundation isClassFromFoundation:self]) return [NSMutableArray arrayWithArray:keyValuesArray];
...
- (NSMutableDictionary *)mj_keyValuesWithKeys:(NSArray *)keys ignoredKeys:(NSArray *)ignoredKeys
{
    ...
    // TODO: 报错机制不好, 需要重做
    MJExtensionAssertError(![QNFoundation isClassFromFoundation:[self class]], (NSMutableDictionary *)self, [self class], @"不是自定义的模型类")
    ...
    [clazz mj_enumerateProperties:^(QNProperty *property, BOOL *stop) {
        @try {
            ...
            if ([clazz mj_isReferenceReplacedKeyWhenCreatingKeyValues]) {
                ...
                [propertyKeys enumerateObjectsUsingBlock:^(QNPropertyKey *propertyKey, NSUInteger idx, BOOL *stop) {
                    // 下一个属性
                    QNPropertyKey *nextPropertyKey = nil;
                    if (idx != keyCount - 1) {
