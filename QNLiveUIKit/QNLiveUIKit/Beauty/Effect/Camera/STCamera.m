...
@interface STCamera () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate, AVCaptureMetadataOutputObjectsDelegate>
...
- (void)setISOValue:(float)value exposeDuration:(int)duration{
    ...
    if ([self.videoDevice lockForConfiguration:&error]) {
        if (duration) {
           [self.videoDevice setExposureModeCustomWithDuration:CMTimeMake(1, duration) ISO:currentISO completionHandler:nil];
        }
