...
{
    _filterStrength.text = [NSString stringWithFormat:@"%d", (int)(slider.value * 100)];
    if (self.delegate) {
        [self.delegate sliderValueDidChange:slider.value];
    }
}

- (void)filterSliderValueChangedEnd:(UISlider *)slider{
    if (self.delegate) {
        [self.delegate sliderValueDidChangeEnd:slider.value];
    }
}
...
- (void)updateSliderValue:(float)value
{
    ...
    _filterStrength.text = [NSString stringWithFormat:@"%d", (int)(value * 100)];
}
