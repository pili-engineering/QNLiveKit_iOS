...
-(void)efGeneratAllDataSourceWithCallback:(EFDataSourceGeneratorCallback)callback {
    ...
    __block NSArray <EFMaterialGroup *> * materialGroups = [NSArray array];
    dispatch_group_enter(group);
    __block bool flag =true;
    [EFRemoteDataSourceHelper efFetchAllRemoteGroupsWithMaterialsOnSuccess:^(NSArray<EFMaterialGroup *> *arrMaterialGroups) {
        ...
        [self efGeneratAllDataSourceWithCallback:callback];
    }];
...
-(instancetype)_efPackagingAllLocalStickersDatasource:(EFDataSourceModel **)rootModel {
    ...
    for (NSInteger i = 0; i < localStickerZips.count; i ++) {
        ...
        EFDataSourceModel * model1 = [EFDataSourceModel yy_modelWithDictionary:local1ModelDict];
        model1.efIsLocal = YES;
        [local1Array addObject:model1];
        
        EFDataSourceModel * model2 = [EFDataSourceModel yy_modelWithDictionary:local2ModelDict];
        model2.efIsLocal = YES;
        [local2Array addObject:model2];
    }
    
    NSString * path = [[NSBundle mainBundle] pathForResource:@"LocalStickers" ofType:@"bundle"];
    NSFileManager * fManager = [[NSFileManager alloc] init];
    if (![fManager fileExistsAtPath:path]) [fManager createDirectoryAtPath:path withIntermediateDirectories:YES attributes:nil error:nil];
    ...
    for (NSInteger i = 0; i < arrFiles.count; i ++) {
        NSString * stickerZip = arrFiles[i];
        NSString * stickerName = [stickerZip stringByReplacingOccurrencesOfString:@".zip" withString:@""];
        
        NSString *imageName = @"none";
        if ([UIImage imageNamed:[NSString stringWithFormat:@"LocalStickers.bundle/%@.%@", stickerName, EFSuffixTypePng]]) {
            imageName = [NSString stringWithFormat:@"LocalStickers.bundle/%@.%@", stickerName, EFSuffixTypePng];
        } else if ([UIImage imageNamed:[NSString stringWithFormat:@"LocalStickers.bundle/%@.%@", stickerName, EFSuffixTypeJpg]]) {
            imageName = [NSString stringWithFormat:@"LocalStickers.bundle/%@.%@", stickerName, EFSuffixTypeJpg];
        }
        ...
        NSDictionary * filterDict = @{
            ...
            @"type": @((local1Model.efType << 5) | (1 << 4)),
            @"imageName": imageName,
            @"route": @(local1Model.efRoute | (currentCount + i + 1))
        };
        ...
        for (NSInteger j = 0; j < makeupModel.efSubDataSources.count; j ++) {
            ...
            if (!detailMakeupModel.efSubDataSources || detailMakeupModel.efSubDataSources.count == 0) {
                ...
                NSMutableDictionary * datasourceDict = [@{
                    ...
                    @"efStrength": ([EFGlobalSingleton sharedInstance].isMale && i == 0 && j == 5) ? @40 : makeupDefaultValues[i][j], // 男生磨皮40
...
//                    whiten_gif.zip
    ...
    NSPredicate * nvshenPredicate;
    if ([EFGlobalSingleton sharedInstance].isMale) {
        nvshenPredicate = [NSPredicate predicateWithFormat:@"SELF.efName == %@", @"qise"];
    } else {
        nvshenPredicate = [NSPredicate predicateWithFormat:@"SELF.efName == %@", @"wanneng"];
    }
    ...
    if (styleZiranDefaultModel) {
        NSDictionary * defaultStyleDict = @{
            ...
            @"efAction": @1,
            @"efStrength": @([EFGlobalSingleton sharedInstance].isMale ? (30 << 8 | 30) : (50 << 8 | 50))
        };
