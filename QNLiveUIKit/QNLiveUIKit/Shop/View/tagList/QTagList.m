...
{
    Class tagClass = _tagClass?_tagClass : [QTagButton class];
  
    // 创建标签按钮
    QTagButton *tagButton = [tagClass buttonWithType:UIButtonTypeCustom];
    if (_tagClass == nil) {
...
{
    // 获取对应的标题按钮
    QTagButton *button = self.tags[tagStr];
...
- (void)updateTagButtonFrame:(NSInteger)i extreMargin:(BOOL)extreMargin
{
    ...
    // 获取当前按钮
    QTagButton *tagButton = self.tagButtons[i];
    // 判断是否设置标签的尺寸
    if (_tagSize.height == 0) { // 没有设置标签尺寸
        // 自适应标签尺寸
        // 设置标签按钮frame（自适应）
