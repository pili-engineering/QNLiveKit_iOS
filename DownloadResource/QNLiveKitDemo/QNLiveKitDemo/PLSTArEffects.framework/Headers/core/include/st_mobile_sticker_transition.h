...
/// @brief 贴纸part的播放状态类型
typedef enum {
    ST_AS_PAUSED_FIRST_FRAME = 1,   ///< 暂停在第一帧状态
    ST_AS_PLAYING = 2,              ///< 正常播放渲染状态
    ST_AS_PAUSED = 3,               ///< 暂停播放渲染状态
    ST_AS_PAUSED_LAST_FRAME = 4,    ///< 暂停在最后一帧状态
    ST_AS_INVISIBLE = 5             ///< 不可见状态
} st_animation_state_type;

/// @brief 贴纸package的播放状态类型
typedef enum {
	ST_AS_BEGIN = 1,                ///< 素材包开始状态
	ST_AS_END = 2,                  ///< 素材包结束状态
    ST_AS_TERMINATED = 3            ///< 素材包终止状态
} st_package_state_type;

/// @brief 事件类型
typedef enum {
    ST_EVENT_ANIMATION = 1,         ///< 贴纸动画状态改变触发事件
    ST_EVENT_HUMAN_ACTION = 2,      ///< 人脸动作触发事件
    ST_EVENT_CUSTOM = 3             ///< 自定义触发事件
...
//    st_animation_state* pre_state;    ///< 前置状态
    int pre_state_module_id;            ///< 前置module id
    st_animation_state_type pre_state;  ///< 前置状态
