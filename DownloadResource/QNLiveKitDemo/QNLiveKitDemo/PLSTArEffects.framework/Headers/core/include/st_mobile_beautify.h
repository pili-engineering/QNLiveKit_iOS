/**
 * @file st_mobile_beautify.h
 *
 * @brief
 * 美颜接口主要包括了基础美颜，微整形，美型等相关功能，相关接口定义在st_mobile_beautify.h文件中
 * 目前该接口支持RGBA纹理格式或Buffer数据输入，具体细节请参考接口文档或示例代码。
 *
 * 调用示例：
 * st_result_t ret = ST_OK;
 * st_handle_t handle = NULL;
 * ret = st_mobile_beautify_create(&handle);
 *
 * st_mobile_beautify_setparam(handle_beautify, ST_BEAUTIFY_CONTRAST_STRENGTH, 0.5f);
 * ret = st_mobile_beautify_process_and_output_texture(handle, in_tex, width, height,
 ...
 *
 * @note
 * st_mobile_beautify.h 在8.0版本已经被废弃，为了获取更高性能或者更多新功能，请使用 st_mobile_effect.h
 * */
...
typedef enum {
    ...
    ST_BEAUTIFY_SHRINK_JAWBONE_RATIO = 19,              ///< 瘦下颔骨比例， [0, 1.0], 默认值0.0， 0.0不做瘦颧骨
    ...
    ST_BEAUTIFY_3D_SHRINK_ROUND_FACE_RATIO = 37,        ///< 圆脸瘦脸比例， [0, 1.0], 默认值0.0， 0.0不做瘦脸
    ST_BEAUTIFY_3D_SHRINK_LONG_FACE_RATIO = 38,         ///< 长脸瘦脸比例， [0, 1.0], 默认值0.0， 0.0不做瘦脸
    ST_BEAUTIFY_3D_SHRINK_GODDESS_FACE_RATIO = 39,      ///< 女神瘦脸比例， [0, 1.0], 默认值0.0， 0.0不做瘦脸
    ST_BEAUTIFY_3D_SHRINK_NATURAL_FACE_RATIO = 40,      ///< 自然瘦脸比例， [0, 1.0], 默认值0.0， 0.0不做瘦脸
    ST_BEAUTIFY_SHRINK_WHOLE_HEAD_RATIO = 41,           ///< 整体缩放小头部比例, [0, 1.0], 默认值0.0, 0.0不做整体小头效果
...
/// @brief 美颜配置项
typedef enum {
    ST_BEAUTIFY_ENABLE_WHITEN_SKIN_MASK,     ///< 是否为美白开启皮肤分割，默认不启用， 0 表示不启用， 非0表示启用
} st_beautify_config_t;

/// @brief 设置美化参数,默认包含所有功能参数和强度参数
/// @param [in] handle 已初始化的美化句柄
/// @param [in] config 美化配置项关键字
/// @param [in] val 参数取值
/// @return 成功返回ST_OK, 错误则返回错误码,错误码定义在st_mobile_common.h中,如ST_E_FAIL等
ST_SDK_API st_result_t
st_mobile_beautify_set_config(st_handle_t handle, st_beautify_config_t config, float val);

/// @brief 设置美化参数,默认包含所有功能参数和强度参数
/// @param [in] handle 已初始化的美化句柄
/// @param [in] type 美化配置项关键字
/// @param [out] val 参数取值
/// @return 成功返回ST_OK, 错误则返回错误码,错误码定义在st_mobile_common.h中,如ST_E_FAIL等
ST_SDK_API st_result_t
st_mobile_beautify_get_config(st_handle_t handle, st_beautify_config_t config, float *val);
