...
#import "QNIMPushServiceProtocol.h"
...
typedef enum {
    QNIMPushMessageDirectionUp = 0, // 取更旧消息
    QNIMPushMessageDirectionDown, // 取更新消息
} QNIMPushMessageDirection;


@class QNIMMessageObject;
@class QNIMError;
@class QNIMPushUserProfile;

@interface QNIMPushService : NSObject

- (void)addDelegate:(id<QNIMPushServiceProtocol>)aDelegate;

- (void)addDelegate:(id<QNIMPushServiceProtocol>)aDelegate delegateQueue:(dispatch_queue_t)aQueue;

- (void)removeDelegate:(id<QNIMPushServiceProtocol>)aDelegate;

+ (instancetype)sharedOption;

/// 初始化推送sdk。在仅使用推送的情况下使用该接口初始化推送sdk。在同时使用IM功能的时候直接在QNIMClient调用登陆功能即可。config对象初始化的时候需要传入平台类型和设备id。
/// @param alias 推送初始化使用的当前用户别名
/// @param QNIMToken 推送初始化的时候App传入的使用的用户的token。
- (void)startWithAlias:(NSString *)alias QNIMToken:(NSString *)QNIMToken;
...
- (void)getPushProfileForceRefresh:(BOOL)forceRefresh
                        completion:(void (^)(QNIMPushUserProfile *profile ,QNIMError *aError))aCompletionBlock;
...
- (void)getTagsByOperationId:(NSString *)operationId
              withCompletion:(void(^)(NSArray *tags, QNIMError *error))aCompletionBlock;
...
- (void)loadLocalPushMessagesFromMessageId:(long long)reMsgId
                             ...
                             directionType:(QNIMPushMessageDirection)directionType
                                completion:(void(^)(NSArray *messageList, QNIMError *error))aCompletionBlock;

/// 添加聊天监听者
/// @param listener 监听者
- (void)addPushListener:(id<QNIMPushServiceProtocol>)listener;

/// 移除聊天监听者
/// @param listener 监听者
- (void)removePushListener:(id<QNIMPushServiceProtocol>)listener;
