/**
 * @file st_mobile_sticker.h
 *
 * @brief
 * 贴纸接口主要包括了2D/3D贴纸相关功能，相关接口定义在st_mobile_sticker.h文件中
 * 目前该接口支持RGBA纹理格式数据输入，具体细节请参考接口文档或示例代码
 *
 * 代码示例：
 * st_result_t ret = ST_OK;
 * st_handle_t handle = NULL;
 * ret = st_mobile_sticker_create(&handle);
 *
 * ret = st_mobile_sticker_change_package(handle, "../sticker.zip", NULL);
 * ret = st_mobile_sticker_process_and_output_texture(handle, in_tex, width, height,
 *                                                    ST_CLOCKWISE_ROTATE_0, ST_CLOCKWISE_ROTATE_0, 0, NULL, NULL,
 *                                                    out_tex, NULL, 0);
 *
 * st_mobile_sticker_destroy(handle);
 * handle = NULL;
 *
 * @note
 * st_mobile_sticker.h 在8.0版本已经被废弃，为了获取更高性能或者更多新功能，请使用 st_mobile_effect.h
 **/
...
typedef enum {
    ...
    ST_MODULE_FACE_MATTING = 11,    ///< 抠脸模块
    ST_MODULE_SKYBOX = 12,          ///< 天空盒模块
    ST_MODULE_CATCH_BUTTERFLY = 13, ///< 捕蝴蝶模块
    ST_MODULE_FACE_STRETCH = 14,    ///< 人脸拖拽模块
    ST_MODULE_DOUBLEGANGER = 15,    ///< 影分身模块
    ST_MODULE_MASK_FILL = 16,       ///< 染发模块
    ST_MODULE_HEAD_ANIMATION = 17,  ///< 大头模块
    ST_MODULE_3D_FACE_MATTE = 18,   ///< 3D人脸抠图
    ST_MODULE_TRYON = 19,           ///< 试妆模块
	ST_MODULE_GAN = 20,             ///< Gan模块
    ST_MODULE_SEGMENT = 21,         ///< segment模块
	//ST_MODULE_GAN_STYLEBLEND = 19 ///< Gan风格融合模块
} st_module_type;

/// @brief 创建贴纸句柄
/// @param[out] handle 贴纸句柄, 失败返回NULL
...
ST_SDK_API st_result_t
st_mobile_sticker_process_and_outout_texture_both(
st_handle_t handle,
unsigned int texture_src, int image_width, int image_height,
st_rotate_type rotate, st_rotate_type frontRotate, bool need_mirror,
st_mobile_human_action_t* human_action,
